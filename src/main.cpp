...
void saveOdo() {
    File odoFile = SFUD.open("/odo.txt", FILE_WRITE);
    if (odoFile) {
        odoFile.print(odoAmount, 2);
        odoFile.close();
    }
}
...
void setup() {
  ...
  // 读取 ODO (from SFUD file)
  SFUD.begin();
  File odoFile = SFUD.open("/odo.txt", FILE_READ);
  if (odoFile) {
      String odoStr = odoFile.readStringUntil('\n');
      odoAmount = odoStr.toFloat();
      odoFile.close();
  } else {
      odoAmount = 0;
  }
